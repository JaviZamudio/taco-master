...
func _ready() -> void:
	...
	$NewOrderTimer.start(NEW_ORDER_INTERVAL)
	$NewOrderTimer.timeout.connect(_on_NewOrderTimer_timeout)

	# Add placeholders for the maximum number of orders
...
func add_order():
	if orders.size() >= MAX_ORDERS:
		print("Maximum number of orders reached. Cannot add more.")
		return
...
func add_order_card(order: Order):
	...
	# Get OrderCards children and find the first placeholder, delete it and add the card in its index
	var order_cards = $OrderCards.get_children()
	for i in range(order_cards.size()):
		var child = order_cards[i]
		if child.name.begins_with("Placeholder"):
			child.queue_free()
			$OrderCards.add_child(card_instance)
			$OrderCards.move_child(card_instance, i)
			break
...
func add_card_placeholder(idx: int=0):
	...
	$OrderCards.add_child(placeholder_node)
	$OrderCards.move_child(placeholder_node, idx)
...
func handle_order_completed(order: Order) -> void:
	...
	# Get the index of the completed order card, remove it, and add a new placeholder in its place
	var order_cards = $OrderCards.get_children()
	for i in range(order_cards.size()):
		var child = order_cards[i]
...
func _on_NewOrderTimer_timeout() -> void:
	if orders.size() < MAX_ORDERS:
		add_order()
